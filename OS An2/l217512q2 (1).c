...
int takeinput(char str[], int *cmdcount, char history[10][100])//Taking input of commands
{
	...
	//-----------------------------------------
		...
		if (strcmp(str, "!!\n") == 0) 
		{
			...
			{
				if(size<10)
					strcpy(str,history[size - 1]); // copy last one
				else
					strcpy(str,history[9]); // copy last one
			}
		}
		else 
		{
...
int main(char argc, char*argv[])
{
	...
	int cmdcount = 0;
	char history[10][100];
	while(takeinput(str,&cmdcount, history))
	{
		if (strcmp(str, "again") != 0)
		{
...
// sleep(3);
		...
		//		 printf(" %s, ", args[i]);
           ...
           //             	printf("STR SIZE %d\n",strsize);
                        	...
                        	for(i = 0; i < (l + p); i++)
                        	{
                          		while(strcmp(args[j],"1>") != 0 && strcmp(args[j],">") != 0 && strcmp(args[j],"|") != 0 && strcmp(args[j],"<") != 0 && j < argcount )
                          			j++;
...
return 0;
          ...
          //---l217512 abdullah dar os an 2 q2
